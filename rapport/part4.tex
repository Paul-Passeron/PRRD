...
Seuls les nœuds non-vides sont stockés. Par exemple, pour un nœud \texttt{i} avec sous-arbre gauche vide, on met \texttt{i} dans \texttt{lchd} et \texttt{lmdt} (idem à droite avec \texttt{rchd} et \texttt{rmdt}).
...
Pour vérifier l'algorithme on utilise une liste globale \texttt{trace} qui enregistre les nœuds visités. On doit prouver donc deux choses : (1) la trace finale contient tous les nœuds dans le bon ordre, et (2) l'arbre final est revenu à son état initial. 
