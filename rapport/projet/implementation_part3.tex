...
Les paramètres fantômes \lstinline{ls} et \lstinline{k} servent à savoir où l'on est dans la forme de la liste. La fonction échange les valeurs a la position $k-1$ et a la position \lstinline{ls.count} - $k$ et ensuite restore le pointeur à la position $k-1$.
...
/*@ ghost (list_shape ls, int k) */
{
    lst_t nfp;
    if (fp == qp) {
        // La taille de la liste est paire, on est arrive a la fin
        back_again(bp, sp, sp) /*@ ghost(ls, k) */;
    } else {
        nfp = fp->cdr;
        if (sp && fp && nfp == qp) {
            // La taille est impaire, on est a une cellule de la fin
